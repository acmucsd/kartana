...
export default class MeetingNotes extends Command {
  constructor(client: BotClient) {
    const definition = new SlashCommandBuilder()
      ...
      .addStringOption((option) => option.setName('date').setDescription("MM/DD/YYYY (Defaults to today's date)"))
      .addStringOption((option) =>
        option.setName('shortlink').setDescription('Shortens the link to acmurl.com/{shortlink}!'),
      )
    ...
    try {
      ...
      if (error.message === 'error:keyword') {
        try {
          ...
          const [, newURL] = await handleExistingACMURL(shortlink, longlink, linkTitle, config);
          return newURL;
        } catch (e2) {
