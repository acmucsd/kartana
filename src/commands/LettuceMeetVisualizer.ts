...
export default class LettuceMeetVisualizer extends Command {
  constructor(client: BotClient) {
    const definition = new SlashCommandBuilder()
      ...
      .addStringOption((option) =>
        option.setName('code').setDescription('Lettuce Meet board code (code at the end of url)').setRequired(true),
      )
      .addNumberOption((option) =>
        option
          .setName('duration')
          .setDescription('Desired meeting duration (in minutes) [default is 60 minutes]')
          .setRequired(false),
      )
      .addStringOption((option) =>
        option
          .setName('users')
          .setDescription('Emails or names of the users to include in the results')
          .setRequired(false),
      )
      .addNumberOption((option) =>
        option.setName('n').setDescription('Number of results to show (default 5).').setRequired(false),
      )
  ...
  public async run(interaction: CommandInteraction): Promise<void> {
    ...
    if (filterByArray) {
      pollResponses = pollResponses.filter(
        (response) =>
  ...
  private async getBestTimesForBlock(
    ...
    const bestTimes: { blockStartTime: number; availabilityData: AvailabilityData }[] = availabilityArray.map(
      ([startTime, availabilityData]) => {
        ...
        for (let i = startTime; i < end; i += LETTUCE_MEET_INTERVAL * 60 * 1000) {
          peopleAvailable = peopleAvailable.filter((person) =>
            availabilityMap.get(i)?.peopleAvailable?.includes(person),
          );
        }
  ...
  private async getAvailabilityMap(boardData: BoardData, filterBy?: string[]): Promise<Map<number, AvailabilityData>> {
    ...
    const filteredPollResponses = pollResponses.filter((pollResponse) => {
      ...
      return (
        filterBy.includes(pollResponse.user.email.toLowerCase()) ||
  ...
  private async getUserAvailability(
    ...
    const pollResponses = boardData.data.event.pollResponses;
    const matchedUsers = pollResponses.filter(
      (pollResponse) => pollResponse.user.name === name || pollResponse.user.email === email,
    );
