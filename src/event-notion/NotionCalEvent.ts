...
function parseLocationURL(urlString: string, eventName: string): URL | null {
  ...
  try {
    const fullUrl = urlString.startsWith('acmurl.com') ? `https://${urlString}` : urlString;
...
export const HostFormResponseSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (venue === EventLocationType.NEED_VENUE) {
      if (data['Ideal Venue Choice'] == 'Other') {
        // ideal venue choice defaults to 'Other' when missing
        if (data['Other venue details?'] == '') {
          // either ideal venue choice is missing but you want a venue on campus
          // or you selected other but left no details
          ctx.addIssue({
      ...
      if (!data['Will you need a projector and/or other tech?']) {
        // not really necessary since this value already defaults to 'No' if empty anyways
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
  ...
  .transform(
    (data) =>
      ({
        ...
        bookingStatus:
          data['Where is your event taking place?'] === EventLocationType.NEED_VENUE ? 'Booking TODO' : 'Booking N/A',
      }) as INotionCalEvent,
  );
