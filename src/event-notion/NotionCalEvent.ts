 ...
 */
export default class NotionCalEvent {
  ...
  private hostedEventDatabaseID: string;

  public setHostedEventDatabaseID(id: string): void {
    this.hostedEventDatabaseID = id;
  }
  ...
  private csiFormStatus: 'CSI Form N/A' | 'CSI Form TODO' | 'CSI Form In Progress' | 'CSI Form Done';
  ...
  private avEquipment: 'From Venue' | 'From ECE Department' | 'From University Centers Tech' | 'From ACM' | 'N/A';
  ...
  private fundingSource: ('AS' | 'CSE Department' | 'ECE Department' | 'Internal')[];
  ...
  constructor(formResponse: HostFormResponse) {
    ...
    this.fundingStatus =
      formResponse['Will your event require funding?'] === 'Yes' ? 'Funding TODO' : 'Funding Not Requested';
    ...
    this.uploadToYoutube =
      notionYoutubeAnswer[formResponse['Will you want a recording of your event uploaded to the ACM YouTube channel?']];
    // Add a check to ensure no ill-written URL's are included.
    // Basically, try to fix classic shortenings of ACMURL's, and if there's still a URL parse error
    // Just warn the console and set the URL as null.
    //
