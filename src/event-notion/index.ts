...
export const validateNotionDatabase = (database: GetDatabaseResponse) => {
	...
	const { "Hosted Events Sheet": _, ...notionCalSchemaFiltered } = notionCalSchema;
	const { "Hosted Events Sheet": __, ...databasePropsFiltered } = database.properties;
...
export const validateGoogleSheetsSchema = (headers) => {
	if (!isEqual(headers.sort(), [...googleSheetSchema].sort())) { // sorting because reordering columns shouldn't be able to break this
